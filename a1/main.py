...
class StreetDatabase:
    ...
    def add(self, street_name, coordinates):
        if street_name in self.street_dict.keys():
            print("Error: Unable to add an existing street")
        if len(coordinates) < 2:
            ...
            for idx in range(0, len(coordinates) - 1):
                self.street_dict[street_name]["line segment"][f"{idx}"] = {}
                self.street_dict[street_name]["line segment"][f"{idx}"]["point_start"] = coordinates[idx]
                self.street_dict[street_name]["line segment"][f"{idx}"]["point_end"] = coordinates[idx + 1]
                self.street_dict[street_name]["line segment"][f"{idx}"]["gradient"] = self.calc_gradient(
    ...
    def mod(self, street_name, coordinates):
        if street_name not in self.street_dict.keys():
            print("Error: Unable to modify a new street")
        if len(coordinates) < 2:
            print("Error: A street needs at least two coordinates")
        else:
            self.street_dict[street_name] = {}
    ...
    def rm(self, street_name):
        if street_name not in self.street_dict.keys():
            print("Error: Unable to remove a new street")
        else:
        ...
        if self.is_parallel(g1, g2) is not True:
            ...
            if range_l <= x <= range_h:
                ...
                if dist_1 <= dist and dist_2 <= dist:
                    if (x, y) not in l1 and (x, y) not in l2:
